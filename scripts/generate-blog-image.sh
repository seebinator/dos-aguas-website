#!/bin/bash
# CMS - Blog Post Bild generieren via Pollinations.ai (kostenlos)
...
if [ "$LANGUAGE" == "de" ]; then
    case "$CATEGORY" in
        "KI")
            PROMPT="Futuristic artificial intelligence, neural networks, digital transformation, modern, professional, dark blue background, technological, high quality, abstract"
            ;;
        "Steuern")
            PROMPT="International finance, tax consulting, Germany Mexico business, professional, trustworthy, modern, dark blue background, corporate"
            ;;
        "Odoo")
            PROMPT="ERP software, business management, digital processes, modern, professional, technological, dark blue background"
            ;;
        "Marketing")
            PROMPT="Digital marketing, content creation, social media, modern, creative, professional, dark blue background"
            ;;
        *)
            PROMPT="Business consulting, international company, Germany Mexico, professional, modern, dark blue background, high quality"
            ;;
    esac
else
    case "$CATEGORY" in
        "IA")
            PROMPT="Inteligencia artificial futurista, redes neuronales, transformación digital, moderno, profesional, fondo azul oscuro, tecnológico, alta calidad"
            ;;
        "Impuestos")
            PROMPT="Finanzas internacionales, consultoría fiscal, negocios Alemania México, profesional, confiable, moderno, fondo azul oscuro"
            ;;
        "Odoo")
            PROMPT="Software ERP, gestión empresarial, procesos digitales, moderno, profesional, tecnológico, fondo azul oscuro"
            ;;
        "Marketing")
            PROMPT="Marketing digital, creación de contenido, redes sociales, moderno, creativo, profesional, fondo azul oscuro"
            ;;
        *)
            PROMPT="Consultoría empresarial, empresa internacional, Alemania México, profesional, moderno, fondo azul oscuro, alta calidad"
            ;;
    esac
fi
...
if [ -f "$IMAGE_PATH" ] && [ -s "$IMAGE_PATH" ]; then
    echo "✓ Bild generiert und gespeichert: $IMAGE_PATH"
else
