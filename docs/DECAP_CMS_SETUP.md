...
## Option 2: Vercel + Git Gateway (Empfohlen für Vercel)
...
Da Vercel kein eingebautes Identity hat, nutzen wir eine selbst gehostete Lösung:

#### A. Netlify Git Gateway (auch für Vercel nutzbar)
...
#### B. Oder: Self-hosted Git Gateway

1. Deploye [git-gateway](https://github.com/netlify/git-gateway) auf einem Server
2. Konfiguriere `config.yml` mit deiner Git Gateway URL:
...
backend:
  ...
  base_url: https://dein-git-gateway.com
```

---

## Option 3: GitHub OAuth (Einfach, aber öffentlicher Zugriff)
...
backend:
  ...
  repo: seebinator/dos-aguas-website
  branch: main
  base_url: https://dos-aguas.consulting
  auth_endpoint: oauth
```

### Schritt 3: OAuth Proxy einrichten

Da GitHub OAuth keine clientseitige Authentifizierung erlaubt, benötigst du einen Proxy:

1. Nutze [netlify-cms-github-oauth-provider](https://github.com/vencax/netlify-cms-github-oauth-provider)
2. Oder deploye auf Vercel mit [next-auth](https://next-auth.js.org/)

---

## Empfohlene Lösung für Dos Aguas
...
Die Website läuft dann auf Netlify, und du hast ein vollständiges CMS.
